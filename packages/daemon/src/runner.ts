...
export async function runDaemon(options: {
  ...
  console.log(`[${timestamp()}] Vibepity daemon started (PID: ${process.pid})`);
  ...
  if (options.telegram) {
    ...
    try {
      const { loadConfig } = await import('@vibepity/auth');
      const config = await loadConfig();
      if (config.telegramBotToken) {
        // Dynamic import to avoid loading grammy when not needed
        const telegramModule = await import('@vibepity/telegram');
